...
class BadgeTest {
    ...
    fun `5천 원 이상 1만 원 미만 구매 시, 별 배지를 증정한다`(input: Int) {
        // given
    ...
    fun `1만 원 이상 2만 원 미만 구매 시, 트리 배지를 증정한다`(input: Int) {
        // given
        val money = input.toMoney()
    ...
    fun `2만 원 이상 구매 시, 산타 배지를 증정한다`(input: Int) {
        // given
        val money = input.toMoney()
    ...
    fun `매칭되는 이벤트 배지가 없을 때, 오류를 반환한다`() {
        assertThatThrownBy { Badge.from(Money(1000)) }.isExactlyInstanceOf(IllegalArgumentException::class.java)
    }
}